...
// The jsPsych sketchpad plugin modified to include an eraser (white colour option
// with stroke width preset to 3 times the black colour stroke width)
  ...
  class SketchpadPlugin {
      ...
      setup_event_listeners() {
          ...
          for (const btn of color_btns) {
              btn.addEventListener("click", (e) => {
                  ...
                  // This makes sure the stroke width is different for black and white
                  if (target.getAttribute("data-color") === "white") {
                    this.params.stroke_width = 20;
                    document.getElementById("sketchpad-canvas").style.cursor = "crosshair";
                  }
                  if (target.getAttribute("data-color") === "black") {
                    this.params.stroke_width = 5;
                    document.getElementById("sketchpad-canvas").style.cursor = "default";
                  }
              });
          }
          ...
          window.addEventListener("keypress", this.keyboard_listener)
      }

      keyboard_listener(e) {
        if (e.code === 'KeyU' && !this.display.querySelector("#sketchpad-undo").disabled) {
            this.undo();
        } else if (e.code === 'KeyR' && !this.display.querySelector("#sketchpad-redo").disabled) {
            this.redo();
        }
      }
      ...
      start_draw(e) {
          ...
          this.stroke.push({
              ...
              width: this.params.stroke_width,
              action: "start",
